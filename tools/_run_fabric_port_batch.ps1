...
$OutJson = Join-Path $RepoRoot "branch-fix-propagation-fabric.json"
$Branches = @(
  "fabric/minecraft-1.18.2",
  "fabric/minecraft-1.19.2",
  "fabric/minecraft-1.19.4",
  "fabric/minecraft-1.20.1",
  "fabric/minecraft-1.20.6",
...
function Fix-ElytraClient([string]$Wt) {
  ...
  $client = Join-Path $Wt "src/main/java/dev/attuned/client/UpdraftLiftClient.java"
  if (Test-Path $client) {
...
function Invoke-PortBranch([string]$Branch, [bool]$Retry) {
  ...
  if ($skip) { $r.status = "skip"; $r.detail = $skip; return $r }
  Write-Host "`n######## $Branch ########"
