...
LOADER_PROFILE_RELATIVE_FILE = Path("config/loader-support-profiles.json")
LOADER_PROFILE_TOOL_RELATIVE_FILE = Path("tools/loader_support_profile.py")
...
def _load_loader_profile_tool(root: Path = ROOT):
    tool_path = root / LOADER_PROFILE_TOOL_RELATIVE_FILE
    if not tool_path.is_file():
        raise FileNotFoundError(f"{relative(tool_path, root)} is missing")
    spec = importlib.util.spec_from_file_location("attuned_loader_support_profile", tool_path)
    if spec is None or spec.loader is None:
        raise ImportError(f"cannot import {relative(tool_path, root)}")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
...
def loader_profile_problems(root: Path = ROOT) -> list[str]:
    problems: list[str] = []
    if not (root / LOADER_PROFILE_RELATIVE_FILE).is_file():
        problems.append(f"{LOADER_PROFILE_RELATIVE_FILE}: missing loader support profile registry")
    try:
        profile_tool = _load_loader_profile_tool(root)
        problems.extend(profile_tool.validate_repository(root))
    except (OSError, ImportError, AttributeError, ValueError) as exc:
        problems.append(f"{LOADER_PROFILE_TOOL_RELATIVE_FILE}: {exc}")
    return problems


def check_loader_profile() -> str:
    problems = loader_profile_problems()
    if problems:
        raise CheckFailed("Loader support profile", problems)
    data = _load_loader_profile_tool().load_profiles(ROOT)
    return f"Loader support profile: active {data.get('active_loader', 'unknown')}"
