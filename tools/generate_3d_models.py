...
def update_voxel_report(elements):
	"""Write a local voxel report for regenerated geometry."""
	report_path = ROOT / "build/asset-previews/ocean-relic-trident/ocean_relic_trident_voxel_report.json"
	...
	report.setdefault("reason", "Generated local geometry report.")
...
def cuboid_trident_display():
	"""Display transforms for Minecraft-JSON cuboids, not the centred GLB mesh."""
	return {
...
def regenerate_frostbound():
	...
	model = {
		...
		# Frostbound is a 47-model-unit cuboid; do not inherit the GLB-specific
		# Ocean Relic origin and scales.
