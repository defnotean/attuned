...
final class PassiveEffectRefresher {
	...
	static void refresh(ServerPlayer player, Holder<MobEffect> effect, int duration, int amplifier) {
		refresh(player, effect.value(), duration, amplifier, refreshThreshold(duration));
	}
	...
	static void refresh(ServerPlayer player, Holder<MobEffect> effect, int duration, int amplifier,
		...
		refresh(player, effect.value(), duration, amplifier, refreshThresholdTicks,
	...
	static void refresh(ServerPlayer player, Holder<MobEffect> effect, int duration, int amplifier,
		...
		refresh(player, effect.value(), duration, amplifier, refreshThreshold(duration), ambient, visible, showIcon);
	}
	...
	static void apply(ServerPlayer player, Holder<MobEffect> effect, int duration, int amplifier) {
		player.addEffect(new MobEffectInstance(effect.value(), duration, amplifier,
			PASSIVE_AMBIENT, PASSIVE_VISIBLE, PASSIVE_SHOW_ICON));
	}

	private static void refresh(ServerPlayer player, MobEffect effect, int duration, int amplifier,
