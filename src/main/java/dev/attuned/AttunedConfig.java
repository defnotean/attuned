...
public record AttunedConfig(
	...
	private static <T> MapCodec<T> strictOptionalField(Codec<T> codec, String name, T defaultValue) {
	...
	private record PartyCodecFields(
		...
		private static final MapCodec<PartyCodecFields> MAP_CODEC = RecordCodecBuilder.mapCodec(in -> in.group(
			strictOptionalField(Codec.intRange(MIN_PARTY_MAX_MEMBERS, MAX_PARTY_MAX_MEMBERS),
					"party_max_members", DEFAULT.partyMaxMembers())
				.forGetter(PartyCodecFields::partyMaxMembers),
			strictOptionalField(Codec.floatRange(MIN_PARTY_SHARED_CREDIT_RADIUS, MAX_PARTY_SHARED_CREDIT_RADIUS),
					"party_shared_credit_radius", DEFAULT.partySharedCreditRadius())
				.forGetter(PartyCodecFields::partySharedCreditRadius),
			strictOptionalField(Codec.intRange(MIN_PARTY_SHARED_CREDIT_WINDOW_TICKS, MAX_PARTY_SHARED_CREDIT_WINDOW_TICKS),
					"party_shared_credit_window_ticks", DEFAULT.partySharedCreditWindowTicks())
				.forGetter(PartyCodecFields::partySharedCreditWindowTicks),
			strictOptionalField(Codec.intRange(MIN_PARTY_COOLDOWN_TICKS, MAX_PARTY_COOLDOWN_TICKS),
					"party_invite_ttl_ticks", DEFAULT.partyInviteTtlTicks())
				.forGetter(PartyCodecFields::partyInviteTtlTicks),
			strictOptionalField(Codec.intRange(MIN_PARTY_COOLDOWN_TICKS, MAX_PARTY_COOLDOWN_TICKS),
					"party_invite_cooldown_ticks", DEFAULT.partyInviteCooldownTicks())
				.forGetter(PartyCodecFields::partyInviteCooldownTicks),
			strictOptionalField(Codec.intRange(MIN_PARTY_COOLDOWN_TICKS, MAX_PARTY_COOLDOWN_TICKS),
					"party_create_cooldown_ticks", DEFAULT.partyCreateCooldownTicks())
				.forGetter(PartyCodecFields::partyCreateCooldownTicks),
			strictOptionalField(Codec.BOOL,
			...
			strictOptionalField(Codec.BOOL, "party_hud_enabled", DEFAULT.partyHudEnabled())
				.forGetter(PartyCodecFields::partyHudEnabled),
			strictOptionalField(Codec.BOOL, "party_shared_credit_enabled", DEFAULT.partySharedCreditEnabled())
				.forGetter(PartyCodecFields::partySharedCreditEnabled),
			strictOptionalField(Codec.BOOL, "party_trial_assists_enabled", DEFAULT.partyTrialAssistsEnabled())
				.forGetter(PartyCodecFields::partyTrialAssistsEnabled),
			strictOptionalField(Codec.BOOL, "party_effects_enabled", DEFAULT.partyEffectsEnabled())
				.forGetter(PartyCodecFields::partyEffectsEnabled),
			strictOptionalField(Codec.BOOL, "party_confluence_hints_enabled", DEFAULT.partyConfluenceHintsEnabled())
	...
	private static final Codec<AttunedConfig> BASE_CODEC = RecordCodecBuilder.create(in -> in.group(
	...
	public static final Codec<AttunedConfig> CODEC = Codec.of(BASE_CODEC, AttunedConfig::decodeConfig);

	private static <T> DataResult<Pair<AttunedConfig, T>> decodeConfig(DynamicOps<T> ops, T input) {
		return validatePartyConfigInput(ops, input)
			.flatMap(validInput -> BASE_CODEC.decode(ops, validInput));
	}
	...
	private static <T> DataResult<T> validatePartyConfigInput(DynamicOps<T> ops, T input) {
		JsonElement element = ops.convertTo(JsonOps.INSTANCE, input);
		if (!element.isJsonObject()) {
			return DataResult.success(input);
		}
		JsonObject json = element.getAsJsonObject();
		return validatePartyInt(json, "party_max_members",
				MIN_PARTY_MAX_MEMBERS, MAX_PARTY_MAX_MEMBERS, input)
			.flatMap(validInput -> validatePartyFloat(json, "party_shared_credit_radius",
				MIN_PARTY_SHARED_CREDIT_RADIUS, MAX_PARTY_SHARED_CREDIT_RADIUS, validInput))
			.flatMap(validInput -> validatePartyInt(json, "party_shared_credit_window_ticks",
				MIN_PARTY_SHARED_CREDIT_WINDOW_TICKS, MAX_PARTY_SHARED_CREDIT_WINDOW_TICKS, validInput))
			.flatMap(validInput -> validatePartyInt(json, "party_invite_ttl_ticks",
				MIN_PARTY_COOLDOWN_TICKS, MAX_PARTY_COOLDOWN_TICKS, validInput))
			.flatMap(validInput -> validatePartyInt(json, "party_invite_cooldown_ticks",
				MIN_PARTY_COOLDOWN_TICKS, MAX_PARTY_COOLDOWN_TICKS, validInput))
			.flatMap(validInput -> validatePartyInt(json, "party_create_cooldown_ticks",
				MIN_PARTY_COOLDOWN_TICKS, MAX_PARTY_COOLDOWN_TICKS, validInput));
	}

	private static <T> DataResult<T> validatePartyInt(
			JsonObject json, String key, int min, int max, T input) {
		if (!json.has(key)) {
			return DataResult.success(input);
		}
		try {
			...
			if (value < min || value > max) {
				return DataResult.error(() -> key + " must be between " + min + " and " + max);
			}
			return DataResult.success(input);
		} catch (RuntimeException e) {
			return DataResult.error(() -> key + " must be an integer");
		}
	}

	private static <T> DataResult<T> validatePartyFloat(
			JsonObject json, String key, float min, float max, T input) {
		if (!json.has(key)) {
			return DataResult.success(input);
		}
		try {
			...
			return DataResult.error(() -> key + " must be a number");
		}
	}
