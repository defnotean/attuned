...
/**
 * Loads the editable Blockbench (.bbmodel) mesh and renders it through the legacy
 * 1.18.2 {@link VertexConsumer} API. The modern special-model SPI is absent on
...
public final class BlockbenchMeshSpecialRenderer {
	...
	private List<Triangle> loadTriangles() {
		try (Resource resource = Minecraft.getInstance().getResourceManager().getResource(this.model);
