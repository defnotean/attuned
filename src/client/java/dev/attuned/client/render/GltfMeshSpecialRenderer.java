...
/**
 * Owned {@code attuned:gltf_mesh} renderer ported to the 1.21.1 client API.
 *
 ...
 * {@link Items#TRIDENT} stack carrying Attuned marker data (see
 ...
 * {@link TridentModel}. The 1.21.1 builder {@link VertexConsumer} emits each vertex
 * through {@code addVertex(pose, ..).setColor()..setNormal(pose, ..)} rather than the
 * legacy {@code vertex(..)..endVertex()} chain.
 */
public final class GltfMeshSpecialRenderer implements GltfModelReceiver {
	...
	private void renderVanillaTridentModel(PoseStack poseStack, MultiBufferSource bufferSource,
		...
		tridentModel.renderToBuffer(poseStack, consumer, light, overlay, 0xFFFFFFFF);
		poseStack.popPose();
	}
	...
	private static boolean isTemporaryHarpoon(ItemStack stack) {
		// On 1.21.1 the marker lives in DataComponents.CUSTOM_DATA, so the shared
		// HarpoonBehavior predicate owns the detection instead of legacy ItemStack#getTag.
		return stack != null && stack.is(Items.TRIDENT) && HarpoonBehavior.isTemporaryHarpoon(stack);
	}
