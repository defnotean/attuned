...
/**
 * Owned {@code attuned:gltf_mesh} renderer ported to the legacy 1.20.6 client API.
 *
 ...
 * {@link Items#TRIDENT} stack carrying Attuned marker custom data (see
 * {@link HarpoonBehavior}), so the dispatcher is registered against the trident:
 * marked stacks render the glTF mesh, every other trident falls back to the vanilla
 * {@link TridentModel}.
 *
 * <p>Relative to the 1.18.2 sibling port this generation reuses {@link
 * HarpoonBehavior#isTemporaryHarpoon(ItemStack)} (the marker lives in {@code
 * DataComponents.CUSTOM_DATA} rather than the removed {@code ItemStack.getTag()}) and
 * emits vertices through JOML-backed {@link PoseStack.Pose} overloads.
 */
public final class GltfMeshSpecialRenderer implements GltfModelReceiver {
	...
	private static boolean isTemporaryHarpoon(ItemStack stack) {
		...
		// The marker now lives in DataComponents.CUSTOM_DATA on this generation, so reuse the
		// server-side helper instead of re-reading the removed ItemStack.getTag() NBT root.
		return HarpoonBehavior.isTemporaryHarpoon(stack);
	}
	...
	private static void emit(VertexConsumer vertexConsumer, PoseStack.Pose pose,
		...
		vertexConsumer.vertex(pose.pose(), vertex.x(), vertex.y(), vertex.z())
			...
			.uv2(light)
			.normal(pose, vertex.nx(), vertex.ny(), vertex.nz())
			.endVertex();
	}
}
