...
public final class CircleClientState {
	...
	public static void init() {
		...
		ClientPlayNetworking.registerGlobalReceiver(CircleSnapshotPayload.TYPE, (payload, player, sender) ->
			Minecraft.getInstance().execute(() -> accept(payload)));
		ClientPlayNetworking.registerGlobalReceiver(CircleInvitePromptPayload.TYPE, (payload, player, sender) ->
			Minecraft.getInstance().execute(() -> acceptPrompt(payload)));
		ClientPlayNetworking.registerGlobalReceiver(CirclePingClientPayload.TYPE, (payload, player, sender) ->
			Minecraft.getInstance().execute(() -> acceptPing(payload)));
