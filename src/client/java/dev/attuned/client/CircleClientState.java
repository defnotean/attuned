...
public final class CircleClientState {
	...
	public static void init() {
		...
		ClientPlayNetworking.registerGlobalReceiver(CircleSnapshotPayload.TYPE, (client, handler, buf, sender) -> {
			CircleSnapshotPayload payload = new CircleSnapshotPayload(buf);
			client.execute(() -> accept(payload));
		});
		ClientPlayNetworking.registerGlobalReceiver(CircleInvitePromptPayload.TYPE, (client, handler, buf, sender) -> {
			CircleInvitePromptPayload payload = new CircleInvitePromptPayload(buf);
			client.execute(() -> acceptPrompt(payload));
		});
		ClientPlayNetworking.registerGlobalReceiver(CirclePingClientPayload.TYPE, (client, handler, buf, sender) -> {
			CirclePingClientPayload payload = new CirclePingClientPayload(buf);
			client.execute(() -> acceptPing(payload));
		});
