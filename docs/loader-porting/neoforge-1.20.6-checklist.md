# NeoForge 1.20.6 Loader Checklist
...
NeoForge coordinate: `net.neoforged:neoforge:20.6.139`
...
- [x] Register the explicit owner-state client receiver and invalidate `AttunementReadout` after applying the mirrored state.
...
- `.\gradlew.bat dependencies --configuration compileClasspath --write-locks`: passed and persisted `net.neoforged:neoforge:20.6.139` for `compileClasspath`.
...
- `.\gradlew.bat compileClientJava --no-daemon --stacktrace`: passed after wiring `AttunementStateClient.init()` and `AttunementReadout.invalidate(...)`.
- `.\gradlew.bat build --no-daemon --stacktrace`: passed.
- `.\gradlew.bat runServer --no-daemon --stacktrace`: reached `Attuned initializing`, generated a world, and reached server `Done (5.068s)` on 2026-06-25. The Gradle task later reported failure only because the running server process had to be stopped after startup when stdin was unavailable.
...
- The client receiver now invalidates the cached readout immediately after applying the payload, matching the Forge resonance-bar fix.
...
Implemented:
- The branch registers loot-table callbacks through NeoForge `LootTableLoadEvent`.
- New Attuned pools are appended through `LootTable.addPool(...)`, preserving vanilla table contents.
- On Minecraft 1.20.6/NeoForge 20.6, Attuned does not mutate existing archaeology pools in place because this API line does not expose a clean public builder path for that while preserving the original pool structure.

Still needs runtime proof:
- Open vanilla chest and archaeology loot targets to confirm Attuned additions appear without replacing vanilla loot.
