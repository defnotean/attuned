# Attuned example pack — three Foci, no Java

This is a complete, working example of authoring Foci with **only** a datapack and
a resource pack — no mod code, no JAR rebuild. It ships three Foci that each show a
different authoring lane:
...
2. **Resource pack** — copy the `assets/` folder into a resource pack folder under
   ...
   in **Options > Resource Packs**. Without it the three Foci show raw translation
...
3. In game, run `/reload` to load the datapack, then run **`/attuned validate`**.
   ...
   per-file pass/fail summary. The three example Foci should report a clean pass.
