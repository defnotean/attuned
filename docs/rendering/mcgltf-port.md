...
## Minecraft 1.19.2 Maintenance Port (older generation)

Minecraft 1.19.2 predates the special-model SPI, so the modern render wiring is
not available on this branch. Verified by inspecting the named `1.19.2` jar:
...
- Math is `com.mojang.math` (no JOML). The GLB parser keeps its own immutable
  `V3` / column-major `Mat4` helpers so the scene-node transform math is identical
  to the modern build instead of depending on a version-specific matrix API.
...
- Embedded glTF textures register through `DynamicTexture(NativeImage)` (the
  single-argument 1.19.2 constructor).
...
`ItemStackRenderState` nor a per-entity item render-state model exists in 1.19.2,
and the 1.19.2 `ThrownTridentRenderer` renders the vanilla trident model
