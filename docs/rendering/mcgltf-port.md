...
## Minecraft 1.20.6 (older-gen) Adaptation

This branch is an older generation that does not ship the special-model SPI
(`SpecialModelRenderers` / `NoDataSpecialModelRenderer` / `minecraft:special`) and
has no `SubmitNodeCollector`. Named-jar inspection confirms `RenderType` (singular),
`DynamicTexture`, and Fabric `BuiltinItemRendererRegistry` are available. The
feature therefore keeps the same owned `AttunedGltfModels` GLB parser and shared
reload listener and renders the mesh through the version's real item-render API:

- `GltfMeshSpecialRenderer` registers a Fabric
  `BuiltinItemRendererRegistry.INSTANCE.register(Items.TRIDENT, ...)` dynamic
  renderer. Stacks marked as a temporary harpoon
  (`HarpoonBehavior.isTemporaryHarpoon`, backed by `DataComponents.CUSTOM_DATA`
  rather than the removed `ItemStack.getTag()`) draw the glTF mesh; every other
  trident falls back to the vanilla `TridentModel`.
- The mesh is drawn through `MultiBufferSource.getBuffer(RenderType.entityCutout(texture))`
  with the legacy chained `VertexConsumer` (`vertex(...).color(...).uv(...)
  .overlayCoords(...).uv2(...).normal(...).endVertex()`), keeping the same
  `A, B, C, C` degenerate-quad emit described above.
- Resources are read through `ResourceManager.getResourceOrThrow(...)` and
  `Resource.open()` (this generation removed `Resource.getInputStream()` and the
  single-result `getResource`). `ResourceLocation` uses the
  `new ResourceLocation(namespace, path)` constructor.
- The item definitions stay legacy `minecraft:model` (no `minecraft:special`); the
  two contract tests assert the `BuiltinItemRendererRegistry` wiring instead of a
  data-component special model.
- Vertices and normals are emitted through the JOML-backed `PoseStack.Pose`
  overloads (`vertex(pose.pose(), ...)` / `normal(pose, ...)`), because this
  generation's `VertexConsumer` has no `normal(Matrix3f, ...)` overload.
