...
- Minecraft 1.20.6 needs compatibility code that newer branches do not need, including older HUD, keybind, registry, item-use callback, attribute-modifier, trident-rendering, and payload-codec paths.
...
### Minecraft 1.20.6 compatibility notes
- This release targets Minecraft 1.20.6, Java 21, Fabric Loader 0.19.3+, and Fabric API 0.100.8+1.20.6.
- The branch keeps older 1.20.6-compatible APIs for HUD rendering, keybind registration, registry iteration, item-use callbacks, network codecs, attribute modifiers, and trident item rendering.
- Wellspring and Current Runner use underwater Dolphin's Grace behavior on this branch because Minecraft 1.20.6 lacks the newer `minecraft:water_movement_efficiency` attribute used by later lines.
- The gameplay intent is the same as newer branches, but some implementation details are intentionally branch-local so the 1.20.6 artifact builds and runs on the correct loader/runtime.
