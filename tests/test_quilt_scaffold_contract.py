...
def test_quilt_gradle_plugin_and_dependencies_are_native() -> None:
	...
	# Quilt Loom 1.15.1 (latest release) cannot build unobfuscated Minecraft 26.x,
	# so Fabric Loom stays as the build toolchain while quilt.mod.json metadata and
	# the Quilt entrypoint adapters target Quilt Loader at runtime.
	assert "id 'net.fabricmc.fabric-loom' version \"${loom_version}\"" in build
	assert "org.quiltmc:quilt-loader:${project.quilt_loader_version}" in build

	assert "implementation \"net.fabricmc.fabric-api:fabric-api:${project.fabric_api_version}\"" in build
	...
	assert "loom_version=1.17.11" in props
	assert "fabric_api_version=0.152.1+26.1.2" in props
...
def test_quilt_metadata_replaces_fabric_metadata() -> None:
	...
	assert dependencies["fabric-api"] == ">=0.152.1"
	assert dependencies["minecraft"] == "~26.1.2"
	assert dependencies["java"] == ">=25"
