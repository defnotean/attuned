...
def test_neoforge_build_uses_moddev_and_neoforge_coordinate():
    ...
    assert "minecraft_version=26.1.2" in props
    assert "neo_version=26.1.2.76" in props
    assert "neoforge_version_range=[26.1.2,)" in props
...
def test_neoforge_publish_and_docs_are_loader_scoped():
    build = read("build.gradle")
    checklist = read("docs/loader-porting/neoforge-26.1.2-checklist.md")

    assert 'loaders = ["neoforge"]' in build
    assert "fabric-api" not in build
    assert "NeoForge coordinate: `net.neoforged:neoforge:26.1.2.76`" in checklist
