...
def test_neoforge_build_uses_moddev_and_neoforge_coordinate():
    ...
    assert "minecraft_version=26.2" in props
    assert "neo_version=26.2.0.7-beta" in props
    assert "neoforge_version_range=[26.2,)" in props
...
def test_neoforge_metadata_replaces_fabric_metadata():
    ...
    assert "type=\"optional\"" in metadata
...
def test_neoforge_publish_and_docs_are_loader_scoped():
    build = read("build.gradle")
    checklist = read("docs/loader-porting/neoforge-26.2-checklist.md")

    assert 'loaders = ["neoforge"]' in build
    assert "fabric-api" not in build
    assert "NeoForge coordinate: `net.neoforged:neoforge:26.2.0.7-beta`" in checklist
...
def test_neoforge_26_2_uses_submit_node_geometry_event_for_tremor_outlines():
    events = read("src/client/java/net/fabricmc/fabric/api/client/rendering/v1/level/LevelRenderEvents.java")
    context = read("src/client/java/net/fabricmc/fabric/api/client/rendering/v1/level/LevelRenderContext.java")
    tremor = read("src/client/java/dev/attuned/client/TremorOreOutlines.java")

    assert "SubmitCustomGeometryEvent" in events
    assert "event.getSubmitNodeCollector()" in events
    assert "SubmitNodeCollector submitNodeCollector();" in context
    assert "context.submitNodeCollector().submitShapeOutline(" in tremor
