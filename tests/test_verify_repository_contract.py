...
""",
    ...
    def test_release_claims_report_private_art_workflow_copy(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            platform = root / "docs" / "platform"
            platform.mkdir(parents=True)
            (platform / "modrinth-description.md").write_text(
                "Generated from the private asset workflow.\n",
                encoding="utf-8",
            )
            ...
            self.assertTrue(any("private asset workflow" in problem for problem in problems))
    ...
    def test_version_profile_problems_report_missing_ci_whitespace_gate(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            write_version_profile_fixture(root)
            (root / ".github" / "workflows" / "ci.yml").write_text(
    ...
    def test_png_resources_report_orphan_mcmeta_sidecars(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            textures = root / "src" / "main" / "resources" / "assets" / "attuned" / "textures" / "item"
            textures.mkdir(parents=True)
            (textures / "orphan.png.mcmeta").write_text('{"animation": {"frametime": 2}}\n', encoding="utf-8")
...
""",
    ...
    def test_release_jar_problems_report_private_source_manifest_leakage(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            focus_dir = root / "src" / "main" / "resources" / "data" / "attuned" / "attuned" / "focus"
            focus_dir.mkdir(parents=True)
            ...
            with zipfile.ZipFile(jar_path, "w") as jar:
                ...
                jar.writestr("assets/attuned/textures/item/private-asset-manifest.json", "{}")

            problems = verify_repository.release_jar_problems(root, require=True)

            self.assertEqual(2, len(problems))
            self.assertTrue(any("asset-source-manifest.json" in problem for problem in problems))
            self.assertTrue(any("private-asset-manifest.json" in problem for problem in problems))
