...
### Changed
- **Fabric release naming** - Modrinth and CurseForge uploads now use Fabric: Minecraft 1.20.1 - Attuned 1.5.6+mc1.20.1 as the public version display name so future Forge and NeoForge files can sit beside Fabric builds without ambiguity.
- **Patch-only artifact refresh** - rebuilt the Minecraft 1.20.1 Fabric jar from the already-verified gameplay code with the release-size optimization applied; this patch does not add gameplay content or rebalance Foci.
...
- **This is a full gameplay-polish backport, not a small content-only patch.** The 1.20.1 maintenance line now carries the same player-facing systems added on the newer lines: Circles, party pings, shared contribution rules, richer preset metadata, Deep Lantern support Foci, action-bar routing, Updraft polish, combat feedback cleanup, expanded data-driven behavior palettes, docs, gallery updates, and the regression coverage needed to keep those systems shippable.
...
- **The added tests are intentional release weight.** Circles, shared credit, party assistance, preset import, build metadata, action-bar priority, Deep Lantern content, block-context scans, Luck stacking, damage formulas, and 1.20.1 API compatibility all gained focused checks so the backport is not relying on manual playtesting alone.
...
- **The 1.20.1 compatibility layer adds lines without adding separate features.** This branch keeps the legacy Java 17/Fabric 1.20.1 APIs for HUD mixins, registry access, item NBT, Fabric packet callbacks, `FriendlyByteBuf` payload serialization, old attribute modifier operations, and client rendering while preserving the same gameplay behavior players see on newer branches.
- **Water movement needed a branch-specific fallback.** Minecraft 1.20.1 does not have the newer `minecraft:water_movement_efficiency` attribute, so Wellspring and Current Runner use underwater Dolphin's Grace palette behavior on this line to preserve their swim identity without shipping broken modifiers.
...
### Changed
- **Minecraft 1.20.1 gameplay parity** - this maintenance line now carries the same gameplay-polish systems as `latest` while preserving the Minecraft 1.20.1 Fabric target, Java 17 runtime, legacy packet callbacks, and dependency range.
...
- This maintenance release targets Minecraft 1.20.1, Java 17, Fabric Loader 0.18.4+, and Fabric API 0.92.8+1.20.1; newer Minecraft release lines carry the same gameplay systems through their own compatibility branches.
...
- Lowered the Minecraft 1.20.1 Fabric API build and metadata floor to 0.92.8+1.20.1 after build and runtime smoke verification, matching common 1.20.1 modpack installs.
...
## Attuned 1.5.2+mc1.20.1 - Minecraft 1.20.1 Maintenance Build

### Changed
- Started a dedicated Minecraft 1.20.1 maintenance artifact from the verified 1.20.x compatibility branch.
...
- Release metadata targets Minecraft 1.20.1, Java 17, Fabric Loader 0.19.3, and Fabric API 0.92.9+1.20.1.
