...
- **AI-derived Aspect Focus art** - the new Aspect Foci use image-generated source sheets, saved prompts/provenance, and an import/downscale pipeline into 64x512 animated Minecraft item sheets instead of code-first/manual sprite drawing.
...
### Changed
- **Focus art direction** - new Focus art now follows an image-generation-first workflow with saved prompt/source provenance before import, so future Foci match the existing Attuned theme without relying on code-first sprite drawing.
...
### Internal
- Added contract coverage for the Aspect counter matrix, 12 shipped Aspect Foci, Aspect tooltip strings, image/provenance assets, and behavior-palette documentation so release checks catch missing content before publishing.
